...
int _ph_dct_imagehash(CImg<uint8_t> &src, ulong64 &hash){
...
int ph_dct_imagehash_jpeg(std::FILE *const file, ulong64 &hash){
     CImg<uint8_t> src;
     try {
          src.load_jpeg(file);
     } catch (CImgIOException ex){
...
int ph_dct_imagehash_png(std::FILE *const file, ulong64 &hash){
     CImg<uint8_t> src;
     try {
          src.load_png(file);
     } catch (CImgIOException ex){
